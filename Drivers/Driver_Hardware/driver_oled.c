...
{
    uint8_t len_step;       //光标索引
    long temp_int;          //整数部分
    long temp_calc_len;
    int num;
    float decimal_part;     //小数部分
    ...
    /*显示小数部分*/
    decimal_part = val - temp_int;
    for(num = 0;num < accuracy;num++)
    {
        decimal_part *= 10;
        int current_digit = (int)decimal_part; // 取出整数位
        OLED_PrintSignedVal(x, y, current_digit);       
        x++;        
